...
if uploaded_file:
    ...
    # ✅ Dönüştür: Sayısal değilse hata olmasın
    ts = pd.to_numeric(df[col].dropna(), errors='coerce').dropna().values

    if len(ts) == 0:
        st.error("Selected column does not contain valid numeric values.")
    else:
